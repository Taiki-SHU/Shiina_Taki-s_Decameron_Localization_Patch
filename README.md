...
- 영어, 한국어, 일본어 현지화 모드를 추가합니다.
- 지원되는 세 언어 모두에 AI 보조 번역이 포함되어 있습니다.
- 선택한 번역문 위에 원문 중국어를 표시합니다.
- 스토리 텍스트, 화자 이름, 선택지, UI 텍스트, 백로그, 일곱 장의 격언 카드를 지원합니다.
- 메인 타이틀 화면의 텍스트는 이미지에 포함된 것으로 보여 번역 대상에 포함되지 않습니다.
- 수정 내용을 JSON 파일에 직접 저장할 수 있는 로컬 교정 편집기를 포함합니다.
...
- 英語、韓国語、日本語のローカライズモードを追加します。
- 対応する3言語すべてに、AI支援による翻訳が含まれています。
- 選択した翻訳文の上に、元の中国語テキストを表示します。
- ストーリーテキスト、話者名、選択肢、UIテキスト、バックログ、7枚の箴言カードに対応しています。
- メインタイトル画面のテキストは画像に埋め込まれていると思われるため、翻訳には含まれていません。
- 修正内容をJSONファイルへ直接保存できるローカル校正エディターが含まれています。
